...
// main3.js ) 이 Js파일 내부에서는 main2.js와는 달리 pathname을 이용해서
// 정상적인 방식으로 웹페이지를 접속 했을 때 컨텐츠를 표시하게 했고
// 그렇지 않은 경우 오류를 출력하게 변경하였다.
// 또한 메인페이지로 접속 했을 때 각 내용이 undefined로 출력되는 내용을 수정했다.

// +  ) 이 Js파일 내부에서는 main3.js와는 달리 reddir을 이용해서
// 웹으로 이동할 수 있는 파일목록이 추가될 때마다 자동으로 생성 해주는
// 기능의 관한 방법을 설명하고 있다.
...
var app = http.createServer(function(request,response){
    ...
    // 오류를 반환한다.
    if(queryPath === '/') {
      // main3.js ) queryData.id가 없다는 것은 가장 처음 메인 페이지를 의미한다.
      ...
      if(queryData.id === undefined) {
        ...
        fs.readdir('./data', function(error, filelist){
          ...
          // + ) for문을 통해서 list 목록을 생성하고 list에 html 형식의 맞게 저장했다.
